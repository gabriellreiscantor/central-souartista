 ...
 * Mapped via tsconfig paths: "react-router-dom" -> this file.
 */
import {
...
type LinkProps = Omit<ComponentProps<typeof TSLink>, "to" | "className" | "children"> & {
  to: string;
  replace?: boolean;
  state?: unknown;
  className?: string;
...
// ---------- NavLink (react-router v6 function-className compat) ----------
...
export const NavLink = forwardRef<HTMLAnchorElement, NavLinkProps>(function NavLink(
  { to, end, caseSensitive, className, children, replace, state, ...rest },
  ref,
) {
  const loc = tsLocation();
  const { pathname, search, hash } = parseTo(to);
  ...
  const renderState: NavLinkRenderState = { isActive, isPending: false };
  const resolvedClassName = typeof className === "function" ? className(renderState) : className;
